...
except ImportError:  # SQLite development remains usable before PostgreSQL dependencies are installed.
    psycopg = None
    dict_row = None
...
class RecipientRegistrationRequest(BaseModel):
    ...
    hla_type: str
    urgency: str
    waiting_days: int = Field(ge=0, strict=True)
    hospital: str
    city: str
    diagnosis: Optional[str] = ""

# SQLite remains the local-development default. Set AUTH_DATABASE_URL to use PostgreSQL.
DATABASE_PATH = Path(os.getenv("AUTH_DATABASE_PATH", str(BASE_DIR / "auth_users.sqlite3")))
AUTH_DATABASE_URL = os.getenv("AUTH_DATABASE_URL", "").strip()
AUTH_DATABASE_BACKEND = "postgresql" if AUTH_DATABASE_URL else "sqlite"
PASSWORD_HASH_ITERATIONS = 260_000


def _connect_users_db() -> Any:
    if AUTH_DATABASE_BACKEND == "postgresql":
        if psycopg is None or dict_row is None:
            raise RuntimeError("PostgreSQL authentication requires psycopg[binary]")
...
def _get_user_by_email(email: str) -> Optional[Any]:
    with _connect_users_db() as conn:
        placeholder = "%s" if AUTH_DATABASE_BACKEND == "postgresql" else "?"
        return conn.execute(
            f"SELECT id, email, password_hash, role, created_at, is_active, name, hospital, specialization FROM users WHERE email = {placeholder}",
            (email,),
        ).fetchone()


def _create_user(email: str, password: str, role: str, name: Optional[str] = None, hospital: Optional[str] = None, specialization: Optional[str] = None) -> Any:
    try:
        with _connect_users_db() as conn:
            if AUTH_DATABASE_BACKEND == "postgresql":
                return conn.execute(
                    """
                    INSERT INTO users (email, password_hash, role, is_active, name, hospital, specialization)
                    VALUES (%s, %s, %s, TRUE, %s, %s, %s)
                    RETURNING id, email, role, created_at, is_active, name, hospital, specialization
                    """,
...
def _user_response(user: Any) -> dict:
    created_at = user["created_at"]
    if isinstance(created_at, datetime):
        created_at = created_at.strftime("%Y-%m-%d %H:%M:%S")
    return {
        "id": user["id"], "email": user["email"], "role": user["role"],
        "created_at": created_at, "is_active": bool(user["is_active"]),
    ...
    for email, password, role, name, hospital, specialization in default_users:
        ...
        if user is None:
            ...
            with _connect_users_db() as conn:
                placeholder = "%s" if AUTH_DATABASE_BACKEND == "postgresql" else "?"
                conn.execute(
                    f"UPDATE users SET name = {placeholder}, hospital = {placeholder}, specialization = {placeholder} WHERE email = {placeholder}",
                    (name, hospital, specialization, email),
                )
...
def reset_doctor_password(doctor_id: int, data: PasswordResetRequest, current_user: AuthUser = Depends(require_role("admin"))):
    ...
    with _connect_users_db() as conn:
        placeholder = "%s" if AUTH_DATABASE_BACKEND == "postgresql" else "?"
        doctor = conn.execute(f"SELECT id FROM users WHERE id = {placeholder} AND role = 'doctor'", (doctor_id,)).fetchone()
        if doctor is None:
            raise HTTPException(status_code=404, detail="Doctor not found")
        conn.execute(
            f"UPDATE users SET password_hash = {placeholder} WHERE id = {placeholder}",
            (_hash_password(data.password), doctor_id),
        )
