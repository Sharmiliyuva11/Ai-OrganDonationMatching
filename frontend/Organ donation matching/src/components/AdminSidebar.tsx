import { Activity, BarChart3, FileText, Grid2X2, LogOut, PlusCircle, ShieldCheck, UsersRound, X, type LucideIcon } from 'lucide-react'
...
export default function AdminSidebar({ activePage, isOpen, onClose, onNavigate, onLogout }: SidebarProps) {
  const { role } = useNav()
  const portalLabel = role === 'admin' ? 'Admin Portal' : 'Operations'
      ...
      <aside className={`fixed inset-y-0 left-0 z-40 flex w-[224px] shrink-0 flex-col border-r border-portal-border bg-white transition-transform duration-200 lg:translate-x-0 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="flex h-16 items-center gap-3 border-b border-portal-border px-4">
          ...
          <div className="min-w-0">
            ...
            <div className="text-[10px] text-portal-muted">{portalLabel}</div>
          </div>
        ...
        <nav aria-label="Admin portal navigation" className="flex-1 overflow-y-auto px-3 py-5">
          <p className="px-2 pb-2 text-[10px] font-semibold uppercase tracking-[0.16em] text-portal-muted">Operations</p>
        ...
        <div className="space-y-3 border-t border-portal-border p-3">
          <div className="rounded-xl bg-portal-mint px-3 py-2.5">
            <div className="flex items-center gap-2 text-xs font-semibold text-portal-primary"><span className="h-2 w-2 rounded-full bg-portal-success" />System Healthy</div>
            <div className="mt-1 text-[10px] text-portal-muted">Registry synced · 99.2% uptime</div>
          </div>
          ...
          <div className="sr-only">Signed in as admin</div>
        </div>
      </aside>
    </>
  )
