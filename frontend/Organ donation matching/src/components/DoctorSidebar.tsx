import { Activity, Bot, FileText, Grid2X2, History, LogOut, Search, Sparkles, X, type LucideIcon } from 'lucide-react'
...
export default function DoctorSidebar({ activePage, isOpen, onClose, onNavigate, onLogout }: SidebarProps) {
  const { role } = useNav()
  const portalLabel = role === 'doctor' ? 'Doctor Portal' : 'Clinical'
    ...
    <>
      ...
      <aside className={`fixed inset-y-0 left-0 z-40 flex w-[224px] shrink-0 flex-col border-r border-portal-border bg-white transition-transform duration-200 lg:translate-x-0 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <div className="space-y-3 border-t border-portal-border p-3">
          ...
          <div className="sr-only">Signed in as doctor</div>
        </div>
      </aside>
    </>
  )
